...
import { LayoutDashboard, Calendar, FileBarChart, CheckSquare, LogOut, Menu, X } from "lucide-react";
import { useAuthStore } from "@/store/authStore";
import { useState } from "react";

const NAV_ITEMS = [
  { href: "/officer", icon: LayoutDashboard, label: "Dashboard" },
  { href: "/officer/jobs", icon: CheckSquare, label: "Approve Jobs" },
  { href: "/officer/drives", icon: Calendar, label: "Placement Drives" },
  { href: "/officer/reports", icon: FileBarChart, label: "Reports" },
];
...
export default function OfficerLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: "#eae8e3", color: "#000000" }}>
      <aside className="fixed top-0 left-0 h-full z-40 flex flex-col transition-all duration-500" style={{ width: sidebarOpen ? "260px" : "72px", backgroundColor: "#ffffff", borderRight: "1px solid #e0e0e0" }}>
        <div className="p-5 flex items-center gap-3" style={{ borderBottom: "1px solid #e0e0e0", minHeight: "72px" }}>
          {sidebarOpen && <span className="font-black text-sm tracking-widest uppercase" style={{ fontFamily: "Outfit, sans-serif", color: "#000000" }}>Officer</span>}
        ...
        <nav className="flex-1 p-3 flex flex-col gap-1">
          {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
            const isActive = pathname === href || (href !== "/officer" && pathname.startsWith(href));
            return (
        ...
        <div className="p-3" style={{ borderTop: "1px solid #e0e0e0" }}>
          {sidebarOpen && user && (
            <div className="px-3 py-2 mb-2" style={{ borderBottom: "1px solid #e0e0e0" }}>
              ...
              <p className="text-xs truncate mt-0.5" style={{ color: "#888888" }}>Placement Officer</p>
            </div>
          )}
      ...
      <main className="flex-1 min-h-screen transition-all duration-500" style={{ marginLeft: sidebarOpen ? "260px" : "72px", backgroundColor: "#eae8e3" }}>
        <header className="sticky top-0 z-30 px-8 flex items-center" style={{ height: "64px", backgroundColor: "#ffffff", borderBottom: "1px solid #e0e0e0" }}>
          <p className="text-xs font-semibold tracking-widest uppercase" style={{ color: "#000000" }}>Placement Officer</p>
        </header>
        <div className="page-banner">
          <img src="/banner-officer.jpeg" alt="Officer Portal Banner" />
          <div className="page-banner-overlay">
            <span className="banner-text">Placement Officer</span>
          </div>
        </div>
